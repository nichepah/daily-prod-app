...
public class DateDisplaySSP extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_date_display_ssp);

        Intent intent = getIntent();
        myDate = intent.getStringExtra("ProdDate");
        RequestQueue requestQueue = Volley.newRequestQueue(this);

        final String url = "http://10.151.7.169/php/sail/ssp_date.php?Production_Date="+myDate;
        final Integer imageId = R.mipmap.ic_ssp;
                        ...
                        try{
                           // String HM = response.getString("HM");
                            ...
                            runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    CustomListAdapter myAdapter = new CustomListAdapter(DateDisplaySSP.this, jsonKeys, jsonVals, imageIds);
                                    //myAdapter.printVal();
                                    ListView listView = findViewById(R.id.listViewDateSSP);
